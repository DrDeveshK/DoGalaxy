...
<section class="ud-app">
  <aside class="ud-side">
    ...
    <nav>
      ...
      <a href="<?php echo esc_url(home_url('/venues/')); ?>">Browse</a>
  ...
  <div>
    ...
    <?php if (!$item) : ?>
      <p>No listing on this account (seeker / guest). <a href="<?php echo esc_url(home_url('/venues/')); ?>">Browse</a></p>
    <?php else : $inbox = doswagat_inbox($item->ID); ?>
    <h1>Your Venue</h1>
    <form class="ud-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
      <input type="hidden" name="action" value="doswagat_save">
      <?php wp_nonce_field('doswagat_save', '_n'); ?>
