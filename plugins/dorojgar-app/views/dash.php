...
<section class="ud-app">
  <aside class="ud-side">
    ...
    <nav>
      ...
      <a href="<?php echo esc_url(home_url('/jobs/')); ?>">Browse</a>
  ...
  <div>
    ...
    <?php if (!$item) : ?>
      <p>No listing on this account (seeker / guest). <a href="<?php echo esc_url(home_url('/jobs/')); ?>">Browse</a></p>
    <?php else : $inbox = dorojgar_inbox($item->ID); ?>
    <h1>Your Job</h1>
    <form class="ud-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
      <input type="hidden" name="action" value="dorojgar_save">
      <?php wp_nonce_field('dorojgar_save', '_n'); ?>
