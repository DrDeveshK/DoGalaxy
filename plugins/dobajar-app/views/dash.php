...
<section class="ud-app">
  <aside class="ud-side">
    ...
    <nav>
      ...
      <a href="<?php echo esc_url(home_url('/listings/')); ?>">Browse</a>
  ...
  <div>
    ...
    <?php if (!$item) : ?>
      <p>No listing on this account (seeker / guest). <a href="<?php echo esc_url(home_url('/listings/')); ?>">Browse</a></p>
    <?php else : $inbox = dobajar_inbox($item->ID); ?>
    <h1>Your Listing</h1>
    <form class="ud-form" method="post" action="<?php echo esc_url(admin_url('admin-post.php')); ?>">
      <input type="hidden" name="action" value="dobajar_save">
      <?php wp_nonce_field('dobajar_save', '_n'); ?>
