...
function dobajar_activate(): void {
    add_role('bajar_seller', 'Seller', ['read' => true, 'upload_files' => true]);
    foreach (['join'=>'Join','login'=>'Login','dashboard'=>'Dashboard','listings'=>'Listings','contact'=>'Contact'] as $s=>$t) {
...
add_action('init', 'dobajar_types');
function dobajar_types(): void {
    register_post_type('listing', [
        'labels' => ['name' => 'Listings', 'singular_name' => 'Listing'],
        'public' => true, 'has_archive' => false, 'rewrite' => ['slug' => 'listings'],
        'show_in_rest' => true, 'menu_icon' => 'dashicons-admin-site', 'supports' => ['title','editor','excerpt'],
    ]);
    register_post_type('dobajar_req', [
        'labels' => ['name' => 'Order requests', 'singular_name' => 'Order request'],
        'public' => false, 'show_ui' => true, 'supports' => ['title','editor'],
    ]);
}

add_action('wp_enqueue_scripts', function () {
    wp_enqueue_style('dobajar-app', DOBAJAR_URL . 'assets/app.css', [], '1.0.0');
});

function dobajar_mine($uid = 0) {
    $uid = $uid ?: get_current_user_id();
    if (!$uid) return null;
    $q = new WP_Query(['post_type'=>'listing','author'=>$uid,'posts_per_page'=>1,'post_status'=>['publish','pending','draft']]);
    return $q->have_posts() ? $q->posts[0] : null;
}
function dobajar_need_login() {
    if (!is_user_logged_in()) { wp_safe_redirect(home_url('/login/')); exit; }
}

add_action('admin_post_nopriv_dobajar_register', 'dobajar_register');
add_action('admin_post_dobajar_register', 'dobajar_register');
function dobajar_register() {
    if (!wp_verify_nonce($_POST['_n'] ?? '', 'dobajar_register')) wp_die('bad nonce');
    ...
    $role = sanitize_text_field(wp_unslash($_POST['as_role'] ?? 'bajar_seller'));
    $allowed = ['bajar_seller'];
    if (!in_array($role, $allowed, true)) $role = 'bajar_seller';
    ...
    if ($role === 'bajar_seller') {
        $id = wp_insert_post(['post_type'=>'listing','post_status'=>'pending','post_title'=>$title,'post_author'=>$uid,'post_content'=>'']);
...
add_action('admin_post_nopriv_dobajar_login', 'dobajar_login');
add_action('admin_post_dobajar_login', 'dobajar_login');
function dobajar_login() {
    if (!wp_verify_nonce($_POST['_n'] ?? '', 'dobajar_login')) wp_die('bad nonce');
    $email = sanitize_email(wp_unslash($_POST['email'] ?? ''));
...
add_action('admin_post_dobajar_save', function () {
    if (!wp_verify_nonce($_POST['_n'] ?? '', 'dobajar_save')) wp_die('bad nonce');
    dobajar_need_login();
    $item = dobajar_mine();
...
add_action('admin_post_nopriv_dobajar_req', 'dobajar_req');
add_action('admin_post_dobajar_req', 'dobajar_req');
function dobajar_req() {
    if (!wp_verify_nonce($_POST['_n'] ?? '', 'dobajar_req')) wp_die('bad nonce');
    ...
    $rid = wp_insert_post(['post_type'=>'dobajar_req','post_status'=>'private','post_title'=>$name.' → '.get_the_title($pid),'post_content'=>$msg]);
    if (!is_wp_error($rid)) {
        ...
        if ($owner && ($u = get_userdata($owner))) wp_mail($u->user_email, 'Order request on Do Bajar', $name.' <'.$email.'>\n\n'.$msg);
    }
    wp_safe_redirect(add_query_arg('sent','1', wp_get_referer() ?: home_url('/'))); exit;
}

function dobajar_inbox($item_id) {
    return (new WP_Query(['post_type'=>'dobajar_req','post_status'=>'private','posts_per_page'=>30,'meta_key'=>'item_id','meta_value'=>$item_id]))->posts;
}

add_filter('template_include', function ($t) {
    if (is_front_page()) return DOBAJAR_DIR . 'views/home.php';
    if (is_singular('listing')) return DOBAJAR_DIR . 'views/single.php';
    if (!is_page()) return $t;
    $s = get_post_field('post_name', get_queried_object_id());
    $map = ['join'=>'join.php','login'=>'login.php','dashboard'=>'dash.php','listings'=>'dir.php','contact'=>'contact.php'];
    if (isset($map[$s])) {
        if ($s === 'dashboard') dobajar_need_login();
        return DOBAJAR_DIR . 'views/' . $map[$s];
    }
    return $t;
});
